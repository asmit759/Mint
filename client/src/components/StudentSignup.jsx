...
import { studentRegister } from '../store/authSlice';
...
const StudentSignup = () => {
  ...
  useEffect(() => {
    ...
    const registerUser = async () => {
      ...
      setLoading(true);
      try {
        await dispatch(studentRegister(formData)).unwrap();
        toast.success('Registration Successful!', { position: 'top-center', autoClose: 2000, theme: 'dark', transition: Bounce });
      ...
      <div className="bg-gray-900 rounded-2xl shadow-2xl shadow-indigo-900/50 p-8 w-full max-w-md border border-indigo-800/30">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div>
            ...
            <input type="email" placeholder="Enter your KIIT email"
              className="w-full px-4 py-3 bg-gray-800 border border-indigo-700/50 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200"
          ...
          <div>
            ...
            <input type="password" placeholder="Confirm your password"
              className="w-full px-4 py-3 bg-gray-800 border border-indigo-700/50 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-200"
              {...register('confirm_password', { required: 'Please confirm your password', validate: (value) => value === password || 'Passwords do not match' })} />
